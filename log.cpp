...
void Clog::log_sensor_with_state(const char* name, float value, float min, float max, ZoneState state, float state_scale, const char* state_label) {
    if (level == LOG_CTRL_JSON) {
        Serial.printf("{\"info\":\"sensor\",\"time\":%lu,\"%s\":%.2f,\"%s_min\":%.2f,\"%s_max\":%.2f,\"%s_state\":%d}\n",
                      millis(), name, value, name, min, name, max, name, state);
    } else if (level == LOG_CTRL_ARDUINO_PLOTTER) {
        float view_state = state * state_scale;

        Serial.printf("%s:%.2f,%sMin:%.2f,%sMax:%.2f,%s:%.2f\n",
                      name, value, name, min, name, max, state_label, view_state);
    }
}
